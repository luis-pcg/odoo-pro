...
//                    [--base-url=http://localhost:8090] [--out=<dir>] [--headed]
//                    [--only=01-flow-id,02-other] [--render-only]
//
...
//           element?: "sel"  -> screenshot just that element (close-up), steps[] }
//   step: one of
//     { goto: "/web#action=..." }        navigate (waits for the web client)
//     { gotoRaw: "/report/html/..." }    navigate without waiting for the web
//                                        client, for pages served outside it
...
const baseUrl = arg("base-url", "http://localhost:8090").replace(/\/+$/, "");
...
async function runStep(step, flowId) {
    ...
    if (step.gotoRaw !== undefined) {
        // A page served outside the web client -- a rendered report, the portal --
        // never draws a navbar, so waiting for one is a guaranteed timeout.
        await page.goto(baseUrl + step.gotoRaw, { waitUntil: "load" });
        return;
    }
    if (step.gotoXmlId !== undefined) {
        ...
        // A URL that differs from the current one only in its hash does not
        // navigate: the browser stays on the record captured by the previous
        // flow. The unique query string forces a real load every time.
        await page.goto(`${baseUrl}/web?_r=${Date.now()}#${hash}`, { waitUntil: "domcontentloaded" });
        await page.waitForSelector(".o_main_navbar", { timeout: 60000 });
        return;
    }
...
lines.push(
    "> Manual generado con `tools/manual-generator`: " +
        `\`node capture.mjs --config=configs/${config.module}.json --db=${db}\`. ` +
...
console.log(`\nREADME -> ${join(outDir, "README.md").replace(repoRoot + "/", "")}`);
...
const command = `node capture.mjs --config=configs/${config.module}.json --db=${db}`;
...
if (headed) {
    ...
    const pdfPath = join(outDir, "manual.pdf");
