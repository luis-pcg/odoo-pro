...
DB_NAME="v19_l10n_do_accounting"
RECREATE=false
SKIP_INSTALL=false
for arg in "$@"; do
  case "$arg" in
...
for journal in (sale_journal, purchase_journal):
    for dt in env['l10n_do.account.journal.document_type'].search(
        ...
        print('  %-6s %-14s siguiente=%-4s restantes=%-4s vence=%s' % (
            journal.code, dt.l10n_do_ncf_type, dt.l10n_do_next_sequence,
            dt.l10n_do_sequence_remaining, dt.l10n_do_ncf_expiration_date))
